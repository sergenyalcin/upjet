	...
	cases := map[string]struct {
		...
		"SuccessWrite": {
			...
			args: args{
				...
				cfg: config.DefaultResource("upjet_resource", nil, nil, nil, nil),
				fs: func() afero.Afero {
		...
		"SuccessWithTimeout": {
			...
			args: args{
				...
				cfg: config.DefaultResource("upjet_resource", nil, nil, nil, nil, func(r *config.Resource) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			fp, _ := NewFileProducer(
				...
				Setup{},
				config.DefaultResource("upjet_resource", nil, nil, nil, nil), WithFileSystem(tc.args.fs()),
			)
